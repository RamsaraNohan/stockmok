# StockFlow — Final Requirements Specification v3.0
...
StockFlow shall be implemented as a secure, multi-tenant **Inventory & Procurement Management System** that lets a business create a workspace, manage products, categories, warehouses and stock, manage private suppliers and buyers, create and receive purchase orders, control staff access by role, view analytics that trace to operational data, and optionally collaborate with another StockFlow organization through validated product mappings and connected purchase orders.
...
|---|---|---|
| FR-AUTH-001 | A-MUST | Provide a one-page public StockFlow site explaining Inventory, Procurement, Connected Businesses and analytics, describing only functionality that is actually built. |
...
| FR-DASH-009 | A-MUST | **(new)** Inventory Value is the sum of each product's stored stock value, computed from current purchase cost. StockFlow reports replacement-cost value; weighted-average costing is Release D and this must be stated in the UI and the report. |
...
| Brief requirement | StockFlow evidence |
|---|---|
