# StockFlow — Final Security & RBAC Model v3.0
...
StockFlow must guarantee, and prove by test, that:
...
Every cross-tenant access-control question in StockFlow — "can this buyer read that supplier's catalog?", "is this user a party to that shared order?" — requires exactly the forbidden kind of read, because the answer depends on which of the caller's organizations they are acting for. **That is why zone 4 is closed to clients.** The rule is not a limitation worked around; it is the reason the architecture takes the shape it does.
