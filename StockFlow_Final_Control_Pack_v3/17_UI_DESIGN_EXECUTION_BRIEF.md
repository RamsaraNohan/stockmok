# StockFlow — UI Design Execution Brief v3.0
...
- **Header:** StockFlow mark · organization monogram + name (prominent, not a footnote) · role badge · workspace switcher (only when the user has more than one membership) · notification bell with unread count · user menu.
...
| Empty state | "Add a supplier you order from" | "Connect a StockFlow business to exchange purchase orders" |

**Design principle:** Private must look like a complete, first-class way to work — most real suppliers will never be StockFlow users. Connected is an upgrade, not a correction.

---

# 11. The Product Mapping signature workflow — design in the most detail

This is the screen that distinguishes StockFlow from a generic CRUD project. It deserves the most design attention after the dashboard.
