...
export function InventoryByLocationPanel() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      return totals.map((t) => ({
        warehouseName:
          warehouses.items.find((w) => w.warehouseId === t.warehouseId)?.name ?? 'Unknown',
  ...
  if (error) {
    return (
      <ErrorState message="Please try again later." title="Failed to load inventory by location" />
    );
  }

  if (!data || data.length === 0) {
    return <EmptyState description="No active warehouses found." title="No Locations" />;
  }
