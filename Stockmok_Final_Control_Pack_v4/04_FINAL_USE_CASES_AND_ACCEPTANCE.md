# Stockmok — Final Use Cases & Acceptance v3.0
...
|---|---|
| Visitor | Explore Stockmok and authenticate |
...
| Private Supplier | An external supplier with no Stockmok account |
| Private Buyer | An external customer with no Stockmok account; a directory entry only in Release A |
| Connected Supplier | Another Stockmok organization connected as a supplier |
| Connected Buyer | Another Stockmok organization connected as a buyer |
...
**Preconditions:** no Stockmok identity; the handle `grand-ocean` is available.
**Actor:** Visitor → Owner.
